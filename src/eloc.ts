  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case "--target":
        const target = args[++i];
        if (target !== "js") {
          console.error(`Invalid target: ${target}. Must be: js`);
          process.exit(1);
        }
        options.target = "js";
        break;
...
function printHelp() {
  console.log(`
Elo Compiler (eloc) v${pkg.version} - Compile Elo expressions to JavaScript
...
Options:
  ...
  -t, --target <lang>       Target language: js (default)
...
Examples:
  ...
  eloc -e "NOW + PT2H" -t js -p

  # Compile from file
  eloc input.elo -t js

  # Compile to file
  eloc -e "2 + 3" -t js -f output.js
