...
describe("transform - temporal arithmetic", () => {
  it("transforms date + duration", () => {
    const ir = transform(
      binary("+", dateLiteral("2024-01-15"), durationLiteral("P1D")),
    );
    assert.strictEqual(ir.type, "call");
    if (ir.type === "call") {
      assert.strictEqual(ir.fn, "add");
  ...
  it("transforms datetime + duration", () => {
    const ir = transform(
      binary(
        "+",
        dateTimeLiteral("2024-01-15T10:00:00"),
        durationLiteral("PT1H"),
      ),
    );
    assert.strictEqual(ir.type, "call");
    if (ir.type === "call") {
  ...
  it("transforms duration + duration", () => {
    const ir = transform(
      binary("+", durationLiteral("P1D"), durationLiteral("P2D")),
    );
    assert.strictEqual(ir.type, "call");
    if (ir.type === "call") {
      assert.strictEqual(ir.fn, "add");
...
describe("transform - string operations", () => {
  it("transforms string + string", () => {
    const ir = transform(
      binary("+", stringLiteral("hello"), stringLiteral(" world")),
    );
    assert.strictEqual(ir.type, "call");
    if (ir.type === "call") {
      assert.strictEqual(ir.fn, "add");
...
describe("transform - let expressions", () => {
  it("transforms let with single binding", () => {
    const ir = transform(
      letExpr([{ name: "x", value: literal(1) }], variable("x")),
    );
...
describe("transform - complex expressions", () => {
  ...
  it("transforms TODAY + P1D", () => {
    const ir = transform(
      binary("+", temporalKeyword("TODAY"), durationLiteral("P1D")),
    );
    assert.strictEqual(ir.type, "call");
    if (ir.type === "call") {
      assert.strictEqual(ir.fn, "add");
        ...
        transform(
          letExpr(
            [
              {
                name: "f",
                value: lambda(["x"], functionCall("f", [variable("x")])),
              },
            ],
    ...
    const ir = transform(
      letExpr(
        [
          {
            name: "f",
            value: lambda(["x"], binary("+", variable("x"), literal(1))),
          },
        ],
        functionCall("f", [literal(1)]),
      ),
    ...
    const ir = transform(
      letExpr(
        [
          {
            name: "f",
            value: lambda(["x"], functionCall("abs", [variable("x")])),
          },
        ],
        functionCall("f", [literal(1)]),
      ),
...
describe("transform - type definitions", () => {
    ...
    assert.throws(
      () =>
        transform(
          typeDef(
            "T",
            unionType([typeRef("Int"), typeRef("Foo")]),
            literal(42),
          ),
        ),
      /Unknown type selector: 'Foo'/,
  ...
  it("allows user-defined type in nested typedef", () => {
    ...
    const ir = transform(
      typeDef("A", typeRef("String"), typeDef("B", typeRef("A"), literal(42))),
    );
    assert.strictEqual(ir.type, "typedef");
    if (ir.type === "typedef") {
