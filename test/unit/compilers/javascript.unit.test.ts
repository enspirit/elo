...
function withHelpers(code: string, ...helpers: string[]): string {
  const helperDefs = [...helpers]
    .sort()
    .map((h) => JS_HELPERS[h].replace(/\n\s*/g, " "))
    .join(" ");
...
describe("JavaScript Compiler - String Literals", () => {
  it("should compile simple string", () => {
    assert.strictEqual(
      compileToJavaScript(stringLiteral("hello")),
      wrapJS('"hello"'),
    );
  });
  ...
  it("should compile string with spaces", () => {
    assert.strictEqual(
      compileToJavaScript(stringLiteral("hello world")),
      wrapJS('"hello world"'),
    );
  });
  ...
  it("should escape double quotes in output", () => {
    assert.strictEqual(
      compileToJavaScript(stringLiteral('say "hi"')),
      wrapJS('"say \\"hi\\""'),
    );
  });
  ...
  it("should escape backslashes in output", () => {
    assert.strictEqual(
      compileToJavaScript(stringLiteral("a\\b")),
      wrapJS('"a\\\\b"'),
    );
  });
});
...
describe("JavaScript Compiler - Variables", () => {
  ...
  it("should compile member access on _", () => {
    assert.strictEqual(
      compileToJavaScript(memberAccess(variable("_"), "price")),
      wrapJS("_.price"),
    );
    assert.strictEqual(
...
describe("JavaScript Compiler - Arithmetic Operators (unknown types)", () => {
  it("should use kPow helper for input variable members", () => {
    const ast = binary(
      "^",
      memberAccess(variable("_"), "x"),
      memberAccess(variable("_"), "y"),
    );
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers("kPow(_.x, _.y)", "kPow"),
    );
  });
  ...
  it("should use kAdd helper for input variable members", () => {
    const ast = binary(
      "+",
      memberAccess(variable("_"), "a"),
      memberAccess(variable("_"), "b"),
    );
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers("kAdd(_.a, _.b)", "kAdd"),
    );
  });
...
describe("JavaScript Compiler - Complex Expressions (unknown types)", () => {
  it("should compile (_.a + _.b) * _.c with helpers", () => {
    ...
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers("kMul(kAdd(_.a, _.b), _.c)", "kAdd", "kMul"),
    );
  });
  ...
  it("should compile _.price * _.quantity - _.discount", () => {
    const ast = binary(
      ...
      binary(
        "*",
        memberAccess(variable("_"), "price"),
        memberAccess(variable("_"), "quantity"),
      ),
    ...
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers(
        "kSub(kMul(_.price, _.quantity), _.discount)",
        "kMul",
        "kSub",
      ),
    );
  ...
  it("should compile (_.x + 5) * (_.y - 3) / 2", () => {
    ...
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers(
        "kDiv(kMul(kAdd(_.x, 5), kSub(_.y, 3)), 2)",
        "kAdd",
        "kSub",
        "kMul",
        "kDiv",
  ...
  it("should compile mixed arithmetic and boolean", () => {
    ...
    assert.strictEqual(
      compileToJavaScript(ast),
      withHelpers("kMul(_.total, 1.1) > 1000", "kMul"),
    );
  });
});
...
describe("JavaScript Compiler - Edge Cases", () => {
  it("should handle nested Math.pow calls", () => {
    const ast = binary("^", literal(2), binary("^", literal(3), literal(2)));
    assert.strictEqual(
      compileToJavaScript(ast),
      wrapJS("Math.pow(2, Math.pow(3, 2))"),
    );
  });
...
describe("JavaScript Compiler - Date Arithmetic", () => {
  ...
  it("should compile duration + datetime (commutative)", () => {
    const ast = binary(
      ...
      { type: "datetime", value: "2024-01-15T10:00:00Z" },
    );
    assert.strictEqual(
      compileToJavaScript(ast),
      wrapJS(
...
describe("JavaScript Compiler - Function Calls", () => {
  it("should compile assert with boolean condition", () => {
    const ast = {
      ...
      name: "assert",
      args: [{ type: "literal" as const, value: true }],
    };
    assert.strictEqual(
      compileToJavaScript(ast),
      wrapJS(
  ...
  it("should compile assert with comparison expression", () => {
    ...
    assert.strictEqual(
      compileToJavaScript(ast),
      wrapJS(
        '(function() { if (!(5 > 3)) throw new Error("Assertion failed"); return true; })()',
      ),
    );
  });
